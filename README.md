...
**Artikel-Template**: Das Plugin bringt ein eigenes, self-contained Fallback-Template mit (`deon-ai/entry` — lebt im Plugin, nichts wird in das `templates/`-Verzeichnis der Site geschrieben). Neu angelegte Sections bekommen es direkt gesetzt. Bestehende Sections mit leerem Template werden automatisch repariert; bestehende Sections mit einem gesetzten, aber erkennbar leeren/kaputten Custom-Template nur mit explizitem `{ "fix_template": true }` im Request — eine funktionierende Konfiguration wird nie stillschweigend überschrieben. Response meldet `template`/`previous_template` (`"kept"`/`"set"`/`"fixed"`), jede Änderung ist über `/deon-ai/rollback` rückgängig machbar.
